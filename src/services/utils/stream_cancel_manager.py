            ...
            try:
                ...
                async with self._lock:
                    for conv_id, token in self._active_streams.items():
                        # Remover streams que llevan más de 10 minutos (600 segundos)
                        # Aumentado de 5 minutos para evitar cancelaciones prematuras
                        # durante operaciones largas como extracción de parámetros
                        if (current_time - token.created_at).total_seconds() > 600:
