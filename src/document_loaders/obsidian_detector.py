...
import json
from functools import lru_cache  # ✅ NUEVO: Cache
...
class ObsidianContext:
    ...
    type: Literal["vault", "singlefile", "subset"]
    ...
    def __repr__(self):
        return (
            f"ObsidianContext(type={self.type}, is_obsidian={self.is_obsidian}, "
...
class ObsidianDetector:
    ...
    def _detect_vault(self, vault_path: Path) -> ObsidianContext:
        ...
        # ✅ MEJORA 3: Filtrar archivos ocultos eficientemente
        md_files = [
            f for f in md_files
            if
            not any(p.startswith(self.EXCLUDED_PREFIXES) for p in f.relative_to(vault_path).parts)
        ]
    ...
    @lru_cache(maxsize=32)  # ✅ MEJORA 4: Cache para búsquedas repetidas
    def _find_parent_vault(self, path: Path) -> Optional[Path]:
        ...
        for level in range(self.MAX_PARENT_DEPTH):
            obsidian_dir = current / self.OBSIDIAN_FOLDER
        ...
        self.logger.debug(f"No vault root found after searching {self.MAX_PARENT_DEPTH} levels up")
        return None

    @lru_cache(maxsize=16)  # ✅ MEJORA 5: Cache de configuraciones
    def _read_vault_config(self, vault_root: Path) -> Optional[Dict]:
        ...
        try:
            ...
            # app.json
            app_config_file = vault_root / self.OBSIDIAN_FOLDER / "app.json"
            if app_config_file.exists():
                with open(app_config_file, 'r', encoding='utf-8') as f:
                    config["app"] = json.load(f)

            # workspace.json
            workspace_file = vault_root / self.OBSIDIAN_FOLDER / "workspace.json"
            if workspace_file.exists():
                with open(workspace_file, 'r', encoding='utf-8') as f:
                    config["workspace"] = json.load(f)

            # community-plugins.json
            plugins_file = vault_root / self.OBSIDIAN_FOLDER / "community-plugins.json"
